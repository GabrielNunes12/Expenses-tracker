...
public interface UserRepository extends JpaRepository<User, Long> {
  @Query(value = "SELECT CASE WHEN EXISTS (SELECT 1 FROM users u WHERE u.username = ?1 AND u.password = ?2) THEN true ELSE false END", nativeQuery = true)
  boolean findByLoginAndPassword(String userLogin, String password);
